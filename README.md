...
- DÃ©bloquer 395 succÃ¨s et des titres exclusifs
...
395 succÃ¨s automatiques rÃ©partis en 11 catÃ©gories actives :
...
### ðŸŒŸ Bonus saison (Ã©quilibrÃ©s)

- ðŸŸ¢ **Ã‰meraude** : `+20% XP Battle Pass`
- ðŸ”´ **Pourpre** : `+25% kamas sur les gains`
- ðŸŸ¡ **Turquoise** : `20% de chance de double drop sur pack`
- ðŸŸ  **Ocre** : `+35% XP guilde`
- âšª **Ivoire** : `-25% coÃ»t fusion`
- âš« **Ã‰bÃ¨ne** : `+40% kamas sur /sellcard et /sellduplicates`
...
### âš¡ Ouverture multi-pack /krosmoz

- Ouvre rapidement plusieurs packs d'un mÃªme set (jusqu'Ã  25 en une commande)
- Mode **ðŸŽ² random** disponible (rÃ©partition automatique entre tous les sets jouables)
- En mode random, la pity reste **par set** (chaque pack met a jour uniquement le set reellement ouvert)
- Affichage en **dÃ©filement rapide** puis rÃ©sumÃ© final
- Les cartes sont **cumulÃ©es** (ex: `x2`, `x3`) au lieu d'Ãªtre rÃ©pÃ©tÃ©es
- La consommation de packs est exacte (gratuit si dispo, puis packs du stock)
- Les succÃ¨s RNG de "pack unique" se valident uniquement sur une ouverture simple (pas sur le total d'un multipack)

### ðŸ›’ Achat groupÃ© /buypack

- `/buypack` permet d'acheter **1 Ã  25 packs d'un coup** avec confirmation du total
- Affiche prix unitaire, quantitÃ© et coÃ»t total avant validation
- DÃ©bloque des succÃ¨s dÃ©diÃ©s aux achats groupÃ©s et aux gros lots

### ðŸ”’ SuccÃ¨s cachÃ©s multi

- Nouvelles sÃ©ries cachÃ©es liÃ©es aux gros volumes (achat et ouverture)
- Paliers "gros lot" sur les quantitÃ©s 5, 10, 15, 20 et 25 d'un coup
...
| Commande | Description |
|----------|-------------|
| /krosmoz | Ouvrir 1 Ã  25 packs d'un set ou en mode ðŸŽ² random (1 gratuit/heure puis consommation du stock), avec mode giga-pack cumulÃ© |
| /buypack | Acheter 1 Ã  25 packs d'un coup (800 kamas / pack) avec confirmation |
...
| /achievements | Voir les 395 succÃ¨s |
...
Chaque joueur stocke : inventaire, shiny cards, kamas, pity, achievements, titres, progression, stats, krosmoshop, quï¿½tes, guildId, progression Battle Pass.

Chaque guilde stocke : nom, emoji, meneur, officiers, membres, niveau, XP, quï¿½tes hebdo, stats.
